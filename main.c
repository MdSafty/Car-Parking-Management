...
int parkinfo[3][10] ;  /* a 2-D array to store number of vehicle parked */
int vehcount ;  /* to store total count of vehicles */

void display( ) ;
void changecol ( struct vehicle * ) ;
struct vehicle * ad (  int, int, int ) ;
void del ( struct vehicle * ) ;
void getfreerowcol ( int * ) ;
void getrcbyinfo (  int, int * ) ;
void show( ) ;
...
void menu3()
{
  ...
  system ( "cls" ) ;
    ...
    system ( "cls" ) ;
		...
		printf ( "\n\t3>> Display Vehicles Parked (Order)\n" ) ;
...
struct vehicle * ad (  int num, int row, int col )
{
    struct vehicle *v ;
...
void getrcbyinfo (  int num, int *arr )
{
  int r, c, fromrow = 0, torow = 2 ;
	for ( r = fromrow ; r < torow ; r++ )
	{
...
void close()
{
    printf("\n\n\n\nThis Project is created by Md Safty!");
}
